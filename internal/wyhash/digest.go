...
import (
	"github.com/go-inspire/pkg/internal/runtime"
	"reflect"
	"unsafe"
)
...
func (d *Digest) Write(input []byte) (int, error) {
	...
	if d.length != 0 {
		...
		d.seed = _wymix(runtime.ReadUnaligned64(paddr)^s1, runtime.ReadUnaligned64(add(paddr, 8))^d.seed) ^ _wymix(runtime.ReadUnaligned64(add(paddr, 16))^s2, runtime.ReadUnaligned64(add(paddr, 24))^d.seed)
		d.see1 = _wymix(runtime.ReadUnaligned64(add(paddr, 32))^s3, runtime.ReadUnaligned64(add(paddr, 40))^d.see1) ^ _wymix(runtime.ReadUnaligned64(add(paddr, 48))^s4, runtime.ReadUnaligned64(add(paddr, 56))^d.see1)
	...
	for len(input) > 64 {
		...
		seed = _wymix(runtime.ReadUnaligned64(paddr)^s1, runtime.ReadUnaligned64(add(paddr, 8))^seed) ^ _wymix(runtime.ReadUnaligned64(add(paddr, 16))^s2, runtime.ReadUnaligned64(add(paddr, 24))^seed)
		see1 = _wymix(runtime.ReadUnaligned64(add(paddr, 32))^s3, runtime.ReadUnaligned64(add(paddr, 40))^see1) ^ _wymix(runtime.ReadUnaligned64(add(paddr, 48))^s4, runtime.ReadUnaligned64(add(paddr, 56))^see1)
		input = input[64:]
	}
...
func (d *Digest) Sum64() uint64 {
	...
	for i > 16 {
		seed = _wymix(runtime.ReadUnaligned64(paddr)^s1, runtime.ReadUnaligned64(add(paddr, 8))^seed)
		paddr = add(paddr, 16)
		i -= 16
	}
	...
	case i == 4:
		a = runtime.ReadUnaligned32(paddr)
		// b = 0
		return _wymix(s1^uint64(length), _wymix(a^s1, seed))
	case i < 8:
		a = runtime.ReadUnaligned32(paddr)
		b = runtime.ReadUnaligned32(add(paddr, i-4))
		return _wymix(s1^uint64(length), _wymix(a^s1, b^seed))
	case i == 8:
		a = runtime.ReadUnaligned64(paddr)
		// b = 0
		return _wymix(s1^uint64(length), _wymix(a^s1, seed))
	default: // 8 < i <= 16
		a = runtime.ReadUnaligned64(paddr)
		b = runtime.ReadUnaligned64(add(paddr, i-8))
		return _wymix(s1^uint64(length), _wymix(a^s1, b^seed))
	}
}
...
//func byteToSlice(b [64]byte, length int) []byte {
//	return unsafe.Slice(&b[0], length)
//}
